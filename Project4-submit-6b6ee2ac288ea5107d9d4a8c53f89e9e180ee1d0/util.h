...
typedef struct CPU_State_Struct {
	...
	uint32_t ID_EX_NPC;
	uint32_t ID_EX_REG1;
	uint32_t ID_EX_REG2;	// value of register

	uint32_t ID_EX_REG1_NUM;
	uint32_t ID_EX_REG2_NUM; // number of register

	short ID_EX_IMM;
	unsigned char ID_EX_DEST; // write address 
	unsigned char ID_EX_OPCODE;
	unsigned char ID_EX_FUNCT;
	unsigned char ID_EX_SHAMT;

	//EX_MEM_latch
	//uint32_t EX_MEM_NPC;
	uint32_t EX_MEM_ALU_OUT;
	uint32_t EX_MEM_W_VALUE; // sw
	uint32_t EX_MEM_BR_TARGET;
	uint32_t EX_MEM_BR_TAKE;
	unsigned char EX_MEM_DEST;
	unsigned char EX_MEM_OPCODE; // To distinguish lw and sw
	unsigned char EX_MEM_FUNCT;

	//MEM_WB_latch
	//uint32_t MEM_WB_NPC;
	uint32_t MEM_WB_ALU_OUT;
	uint32_t MEM_WB_MEM_OUT;
	uint32_t MEM_WB_BR_TAKE; // actually, this value will used for determine PC
	unsigned char MEM_WB_DEST;
	unsigned char MEM_WB_OPCODE;
	unsigned char MEM_WB_FUNCT;
...
void            xdump(int set, int way, int blocksize, Queue** cache);
void		init_memory();
void		init_inst_info();
